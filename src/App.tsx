...
export default function App() {
  ...
  const handleCopyShareLink = (letter: any, type: "in" | "out") => {
    if (!letter) return;
    const baseUrl = window.location.origin + window.location.pathname;
    const shareUrl = `${baseUrl}?letterId=${encodeURIComponent(letter.id)}&type=${type}`;
    ...
    const performCopy = async () => {
      try {
        ...
        setCopySuccess(true);
        setToastMessage(`Tautan surat [${letter.letterNumber}] telah disalin ke papan klip.`);
        setTimeout(() => setCopySuccess(false), 2500);
        setTimeout(() => setToastMessage(null), 3800);
        addAuditLog(`Salin Tautan Surat [No: ${letter.letterNumber}] untuk kolaborasi antar departemen`, "Salin Tautan");
      } catch (err) {
        console.error("Gagal menyalin tautan:", err);
        setToastMessage(`Tautan: ${shareUrl}`);
  ...
  const handleDownloadQRCode = async (letter: any, type: "in" | "out") => {
    if (!letter) return;
    setIsDownloadingQR(true);
    try {
      await downloadVerificationQRPNG(letter, type);
      const code = letter.verificationCode || letter.letterNumber || letter.id;
      setToastMessage(`Stiker QR Code Verifikasi (${code}) berhasil diunduh!`);
      setTimeout(() => setToastMessage(null), 4000);
      addAuditLog(`Unduh Stiker QR Code Verifikasi [No: ${letter.letterNumber}] dari Quick View`, "Cetak Surat");
    } catch (err) {
      console.error("Gagal mengunduh QR Code:", err);
      setToastMessage("Gagal mengunduh stiker QR Code verifikasi.");
  ...
  // Intercept deep link URL parameter to auto-open Quick View modal for inter-departmental collaboration
  useEffect(() => {
    if (typeof window !== "undefined" && (lettersIn.length > 0 || lettersOut.length > 0)) {
      const params = new URLSearchParams(window.location.search);
      const letterId = params.get("letterId") || params.get("letter") || params.get("surat");
      const letterType = params.get("type");

      if (letterId) {
        const foundIn = lettersIn.find(l => l.id === letterId || l.letterNumber === letterId || l.verificationCode === letterId);
        const foundOut = lettersOut.find(l => l.id === letterId || l.letterNumber === letterId || l.verificationCode === letterId);

        if (letterType === "in" && foundIn) {
          setSelectedSearchLetter({ letter: foundIn, type: "in" });
        } else if (letterType === "out" && foundOut) {
          setSelectedSearchLetter({ letter: foundOut, type: "out" });
        } else if (foundIn) {
          setSelectedSearchLetter({ letter: foundIn, type: "in" });
        } else if (foundOut) {
          setSelectedSearchLetter({ letter: foundOut, type: "out" });
...
Sistem Otomatis e-Office FORSDIG`;
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-905 flex justify-center items-center w-full" id="main-viewport-stage">
          ...
          >
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <button
                        onClick={() => handleCopyShareLink(selectedSearchLetter.letter, selectedSearchLetter.type)}
                        ...
                        title="Salin tautan langsung untuk kolaborasi antar departemen"
                      >
                        <Share2 className="h-2.5 w-2.5" />
                        <span>{copySuccess ? "Tautan Tersalin!" : "Bagikan Tautan"}</span>
                      </button>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 text-xs md:text-sm">
                        ...
                        <div 
                          dangerouslySetInnerHTML={{ 
                            __html: generateVerificationQR(
                              selectedSearchLetter.letter.verificationCode || selectedSearchLetter.letter.letterNumber || selectedSearchLetter.letter.id,
                              56,
                              56
                            ) 
                          }}
                ...
                {/* Verification QR Code Sticker Download Card (For both Surat Masuk & Surat Keluar) */}
                <div className="border-t border-slate-100 dark:border-slate-800 pt-5">
                  <div className="bg-slate-50 dark:bg-slate-950/30 p-4 rounded-xl border border-slate-200/80 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-3.5">
                      <div 
                        className="p-1.5 bg-white rounded-lg border border-slate-200 dark:border-slate-700 shadow-xs shrink-0"
                        dangerouslySetInnerHTML={{ 
                          __html: generateVerificationQR(
                            selectedSearchLetter.letter.verificationCode || selectedSearchLetter.letter.letterNumber || selectedSearchLetter.letter.id,
                            64,
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 px-2 py-0.5 rounded border border-emerald-200/80 dark:border-emerald-800">
                            QR Verifikasi Fisik
                          </span>
                          <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400 font-bold">
                            {selectedSearchLetter.letter.verificationCode || selectedSearchLetter.letter.letterNumber}
                          </span>
                        </div>
                        <p className="font-bold text-slate-800 dark:text-slate-100 text-xs mt-1">
                          Stiker QR Code Verifikasi Dokumen
                        </p>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                          Unduh gambar QR secara terpisah untuk dicetak &amp; ditempelkan secara manual ke dokumen fisik.
                        </p>
                      </div>
                    </div>
                    <button
                      onClick={() => handleDownloadQRCode(selectedSearchLetter.letter, selectedSearchLetter.type)}
                      disabled={isDownloadingQR}
                      className="w-full sm:w-auto px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-sm shrink-0"
                      title="Unduh file gambar QR Code verifikasi untuk ditempelkan pada fisik dokumen cetak"
                    >
                      <QrCode className="h-3.5 w-3.5" />
                      <span>{isDownloadingQR ? "Mengunduh..." : "Unduh QR Code (PNG)"}</span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 flex flex-wrap gap-3 items-center justify-between shrink-0">
                ...
                <div className="flex flex-wrap gap-2.5 sm:gap-3">
                  <button
                    onClick={() => handleCopyShareLink(selectedSearchLetter.letter, selectedSearchLetter.type)}
                    className={`px-4 py-2 font-bold text-xs rounded-xl transition-all flex items-center gap-1.5 cursor-pointer shadow-sm ${
                      copySuccess 
                        ? "bg-emerald-600 text-white dark:bg-emerald-600 shadow-emerald-500/20" 
                    ...
                    title="Salin tautan unik surat ini untuk memudahkan kolaborasi antar departemen"
                    id="btn-salin-tautan-quickview"
                  >
                    {copySuccess ? <Check className="h-3.5 w-3.5" /> : <Link className="h-3.5 w-3.5" />}
                  ...
                  <button
                    onClick={() => handleDownloadQRCode(selectedSearchLetter.letter, selectedSearchLetter.type)}
                    disabled={isDownloadingQR}
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white font-bold text-xs rounded-xl transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
                    title="Unduh stiker QR Code verifikasi secara terpisah untuk penempelan dokumen fisik"
                    id="btn-unduh-qr-quickview"
                  >
                    <QrCode className="h-3.5 w-3.5" />
                    <span>{isDownloadingQR ? "Proses..." : "Unduh QR Code"}</span>
                  </button>
      ...
      {/* Floating Toast Notification for Collaboration Links */}
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            className="fixed top-5 right-5 z-50 bg-slate-900/95 dark:bg-slate-800/95 text-white backdrop-blur-md px-4 py-3 rounded-2xl shadow-2xl border border-emerald-500/30 flex items-center gap-3 text-xs md:text-sm font-medium"
          >
            <div className="p-1.5 bg-emerald-500/20 text-emerald-400 rounded-xl">
              <Check className="h-4 w-4" />
            </div>
            <div>
              <p className="font-bold text-white text-xs">Kolaborasi Antar Departemen</p>
